...
# ================================================================== #
#  内部工具：从 TABLE_SCHEMAS 自动生成 SQL，外部不需要调用
# ================================================================== #
...
class StockDataManager:
    ...
    核心接口：
        db.save('daily_basic', df)   ← 存任意表，统一入口
        db.fetch('daily_basic', ...) ← 查任意表，统一入口
    """
    ...
    def _init_db(self):
        ...
        with self._get_connection() as conn:
            ...
            conn.commit()
        print(f"✅ 数据库已就绪：{self.db_path}  "
              f"（{len(TABLE_SCHEMAS)} 张表：{', '.join(TABLE_SCHEMAS)}）")
    ...
    def fetch(self, table_name: str,
        ...
        用法示例：
            ...
            db.fetch('daily_basic', ts_code='600519.SH')
            db.fetch('trade_cal')
        """
...
if __name__ == '__main__':
    ...
    # 查询示例
    df = db.fetch('adj_factor', ts_code='000001.SZ',
                  start_date='20220101', end_date='20250201')
