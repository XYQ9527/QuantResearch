"""
Qlib 数据转换模块 export_qlib.py

从最终信号层 step3_signals 读取所有字段，同步到 Qlib .day.bin 格式
采用排除法：除了少数无用字段，其余全部同步

排除字段：
    trade_date      日期列，不是特征
    adj_factor      复权因子，指标已算完不再需要
    （open/high/low/close/vol 在 compute_factors 层已排除，signals 层不存在）

同步字段（signals 全部列 去掉上述2列）：
    前复权价格  adj_open / adj_high / adj_low / adj_close / adj_vol / amount
    ...
    技术指标    macd_dif / macd_dea / macd_bar
                kdj_k / kdj_d / kdj_j
                rsi_6 / rsi_12
...
generate_signals 新增任何字段，本脚本无需改动，自动同步。
...
from config import DB_PATH, SIGNALS_DIR, QLIB_DIR, QLIB_CAL, QLIB_INST, QLIB_FEAT
...
def _write_bin(values: np.ndarray, start_index: int, out_path: str):
    """写入 Qlib .day.bin 格式：4字节小端uint32起始索引 + float32数组"""
...
def dump_instruments():
    ...
    with open(inst_path, 'w') as f:
        for _, row in df.iterrows():
            f.write(f"{row['qlib_code']}\t{row['list_date']}\t{row['delist_date']}\n")
        ...
        for col in sync_fields:
            if col in df.columns:
                _write_bin(
                    ...
                    start_index,
                    os.path.join(feat_dir, f'{col}.day.bin'),
                )
...
def dump_features(calendar: list, max_workers: int = 20):
    ...
    # 从第一只股票读取字段列表（排除法），signals层新增字段自动同步
    sample_path = os.path.join(SIGNALS_DIR, f'ts_code={all_codes[0]}', 'part-0.parquet')
...
def run_dump(max_workers: int = 20):
    t0 = time.time()
    logger.info('=' * 55)
    logger.info('Qlib 数据转换开始（从 step3_signals 读取）')
    ...
    logger.info(f"  qlib.init(provider_uri=r'{os.path.abspath(QLIB_DIR)}')")
